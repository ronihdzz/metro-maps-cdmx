...
FILE_UBICACION_ESTACIONES = recursos.App_Principal.ARCHIVO_UBICACIONES_ESTACIONES
FILE_SEPARACION_ESTACIONES = recursos.App_Principal.ARCHIVO_SEPARACION_ESTACIONES
CARPETA_IMAGENES_ESTACIONES = recursos.App_Principal.CARPETA_IMAGENES_ESTACIONES
...
class TestSum(unittest.TestCase):
    ...
    def test_nombres_columnas_tabla_separacion(self):
        ...
        diferencia_1 = set(nombres_columnas_esperados) - set(
            self.tabla_separacion.columns
        )
        diferencia_2 = set(nombres_columnas_esperados) - set(
            self.tabla_separacion.columns
        )
    ...
    def test_mismos_nombres_estaciones_files(self):
        diferencia_1 = set(self.tabla_ubicacion.nombre) - set(
            self.lista_nombres_estaciones_by_separacion
        )
        diferencia_2 = set(self.lista_nombres_estaciones_by_separacion) - set(
            self.tabla_ubicacion.nombre
        )
    ...
    def test_mismos_nombres_estaciones_imagenes(self):
        ...
        nombres_imagenes_estaciones = [
            nombre[:-4] for nombre in self.lista_nombres_imagenes
        ]

        diferencia_1 = set(self.tabla_ubicacion.nombre) - set(
            nombres_imagenes_estaciones
        )
        diferencia_2 = set(nombres_imagenes_estaciones) - set(
            self.tabla_ubicacion.nombre
        )
