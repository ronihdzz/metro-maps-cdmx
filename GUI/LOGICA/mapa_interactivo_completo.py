...
class BotonCoordenada(QObject):
    ...
    DISENO_ESTADO_SELECCIONADO = (
        "border: 1px solid blue;border-radius:5px; background-color:red;"
    )
...
class MapaInteractivoCompleto(QMainWindow, HuellaAplicacion):
    ...
    def __init__(self, file_lat_long_red: str, ancho: int, alto: int, margen: int):
        ...
        self.fijar_tamano_panel_mapa(ancho=ancho, alto=alto, margen=margen)
    ...
    def cargar_contenido_panel_mapa(self, file_lat_long_red: str) -> None:
        ...
        # ['self.MARGEN_PANEL'  ,  'self.ANCHO_PANEL'-'self.MARGEN_PANEL']
        latitudes_adapatadas = (
            tabla_ubicaciones["lat"] * (self.ANCHO_PANEL - 2 * self.MARGEN_PANEL)
        ) + self.MARGEN_PANEL

        # logitudes representan el eje y (eje vertical)
        # estaran definidos dentro del rango:
        # ['self.MARGEN_PANEL'  ,  'self.ALTO_PANEL'-'self.MARGEN_PANEL']
        longitudes_adaptadas = (
            tabla_ubicaciones["lng"] * (self.ALTO_PANEL - 2 * self.MARGEN_PANEL)
        ) + self.MARGEN_PANEL

        # graficando los puntos
        self.dict_puntos = {}
        for nombre, x, y in zip(
            tabla_ubicaciones.nombre, latitudes_adapatadas, longitudes_adaptadas
        ):
