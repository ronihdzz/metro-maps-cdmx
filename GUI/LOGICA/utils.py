...
def normalizar_por_escalado_de_variables(valores, valor_min, valor_max):

    valores = np.array(valores, dtype=np.float32)
    resultado = (valores - valor_min) / (valor_max - valor_min)
    return resultado
...
class ListaInteractivaNombres(HuellaAplicacion):
    ...
    def __init__(self, lista_nombres_validos: list, cmb_box_mostrador_lista: QComboBox):
        ...
        self.cmb_box.currentIndexChanged.connect(
            lambda index_select: self.validar_nombre_elegido(index_select)
        )
