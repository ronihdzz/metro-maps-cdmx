...
from PyQt5.QtWidgets import QApplication, QComboBox, QCompleter, QMessageBox
...
class MetroMaps(QtWidgets.QWidget, Ui_Form, HuellaAplicacion):
    def __init__(
        ...
        archivo_ubicaciones_estaciones: str,
        carpeta_imagenes_estaciones: str,
        carpeta_audios_estaciones: str,
        audio_frase_inicio: str,
        audio_frase_intermedia: str,
        ancho_panel: int = 2000,
        alto_panel: int = 800,
        margen_panel: int = 20,
    ):
        ...
        self.mapa_red_metro_cdmx.senal_clic_punto_mapa.connect(
            self.mostrar_imagen_estacion_clic
        )
        self.lista_interactiva_estaciones_origen.senal_cambio_nombre.connect(
            self.mostrar_imagen_estacion_origen
        )
        self.lista_interactiva_estaciones_destino.senal_cambio_nombre.connect(
            self.mostrar_imagen_estacion_destino
        )
    ...
    def mostrar_imagen_estacion_clic(self, nombre_estacion):
        ...
        if self.rb_con_sonido.isChecked():
            sonido_nombre_completo = (
                self.carpeta_audios_estaciones + nombre_estacion + ".mp3"
            )
            self.reproductora_sonidos.reproducir_un_sonido(
                nombre_sonido=sonido_nombre_completo
            )
    ...
    def obtener_ruta_mas_cercana(self) -> None:
        ...
        nombre_origen = (
            self.lista_interactiva_estaciones_origen.get_nombre_seleccionado()
        )
        nombre_destino = (
            self.lista_interactiva_estaciones_destino.get_nombre_seleccionado()
        )
        ...
        if nombre_destino and nombre_origen:
            ...
            if ventanaDialogo.clickedButton() == btn_yes:
                ...
                self.mapa_red_metro_cdmx.destacar_puntos_mapa(
                    lista_nombres_puntos=ruta_seguir
                )
    ...
    def reproducir_en_voz_ruta(self, ruta_estaciones: list) -> None:
        ...
        for nombre_estacion in ruta_estaciones:
            ...
            self.reproductora_sonidos.agregar_sonido(
                nombre_sonido=sonido_nombre_completo
            )
            if nombre_estacion != ruta_estaciones[-1]:
                self.reproductora_sonidos.agregar_sonido(
                    nombre_sonido=self.audio_frase_intermedia
                )
